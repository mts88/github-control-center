...
- Plugin order in `.releaserc.json` is load-bearing: the changelog plugin and the npm bump must precede the vsce packaging (the Marketplace and Open VSX "Changelog" tabs render the `CHANGELOG.md` packaged inside the vsix — which is why `.vscodeignore` must not exclude it), which must precede the git commit; the registry-publish exec instances come **after** `@semantic-release/github` so the durable artifact (GitHub Release + vsix) exists before any marketplace is touched.
...
- One provider instance per view — "To Review" → `githubControlCenter.toReview`, "Reviewed" → `githubControlCenter.reviewed`, "My PRs" → `githubControlCenter.mine` (three separate collapsible views in the container) — each rendering its PR list grouped by repo. The Reviewed view is contributed with `"when": "!config.githubControlCenter.toReview.hideReviewed"`, so its visibility is declarative — no context key, no code.
...
- `partitionReviewed` (`poll/filters.ts`, pure) splits the filtered `reviewed` list: fresh approvals (`viewerReviewState === "APPROVED"` and not stale) fill the "Reviewed" view; everything else (`needsAttention`) stays in "To Review", which renders `[...toReview, ...needsAttention]` — reviewed rows after requested ones within each repo group, decorated with the viewer's review state ("you approved", "review stale" — also used for a stale approval, "you requested changes", "you commented", generic "reviewed" fallback for unknown states — the mapping in `tree/PrTreeProvider.ts` must stay total).
- Staleness is computed at poll time: `PR_FIELDS` fetches `viewerLatestReview { state commit { oid } submittedAt }` plus the head commit's `committedDate`, mapped to `IPullRequest.isViewerApprovalStale` through the same `isStaleApproval` rule the details panel uses (the poll uses only the head commit's date, not the details query's max-over-30 — deliberate, the node budget).
- `poll/ApprovalOverlay.ts` (pure, sibling of the trackers) makes an APPROVE move the PR to "Reviewed" immediately: recorded on mutation success (details composer and pending-review submit), applied to the raw snapshot at the top of `refresh()` so panel re-pointing, filters, providers, badge, and trackers see one coherent world; an entry survives GitHub's search-index lag and clears when the server confirms the approval or the head oid moves.
- Only `toReview` feeds `NewPrTracker`, so a reviewed→re-requested transition toasts as a new request — a stale approval sliding back into the To Review view never toasts.
...
- `submitPrReview` (`addPullRequestReview` APPROVE / REQUEST_CHANGES — request changes requires a body, GitHub rule). The composer only takes this path when no pending review exists: with one, all three composer buttons submit the pending review instead (`submitPendingReview`, GitHub "Finish your review" parity — a second `addPullRequestReview` would be rejected, one pending review per user per PR). Detection is two-source with registry precedence: `ReviewController.getKnownPendingReviewId` (tri-state — `undefined` means this window never loaded threads) wins when it has a snapshot, the details query's `pendingReviews` alias (`IPrDetails.pendingReviewCommentCount`, also feeds the composer banner) covers cross-window/pre-reload pending reviews.
...
- **Settings & trust**: every `githubControlCenter.ai.*` key is **machine-scoped** so an untrusted workspace cannot point the extension at an arbitrary binary, inject the response `language` into the trusted system prompt, or push an out-of-enum `model` onto the (win32) command line — `model` is additionally validated against its enum at read time.
  - `ai.backend` (enum, only `claude-code` today) selects the backend and gates `refreshAiAvailability` — an unknown value disables AI instead of falling through to the claude path.
...
- **Badge count follows the `githubControlCenter.badge.*` settings and mirrors the views exactly** — `countToReview` (default on) adds the To Review view's contents (`toReview` + `needsAttention`), `countMine` (default off) the My PRs view, `countReviewed` (default off) the Reviewed view (fresh approvals only). Badge is set to `undefined` (not `{value: 0}`) when the configured count is 0.
