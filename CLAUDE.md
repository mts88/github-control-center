...
- **One** GraphQL request with three aliased searches:
  ...
  - `mine` — `author:@me`.
  - `reviewed` — `reviewed-by:@me -author:@me` (open PRs the user already reviewed whose request was consumed, including stale/dismissed approvals).
- `reviewed` is deduplicated by id against `toReview` (a re-requested PR matches both searches; the active request wins) and its survivors are tagged `isReviewedByMe`.
...
- **The page** (GitHub-like): header with state pill; conversation timeline (issue comments + review summaries + commits, chronological — consecutive commits collapse into one compact avatar+list group, capped at the last 30); merge box (review decision, checks, mergeable, merge button with repo-allowed methods); composer (Comment / Approve / Request changes); sidebar (reviewers — a stale approval is flagged "· stale" in yellow via `isStaleApproval`, the single rule shared with the Approve-button guard: non-head commit oid OR `submittedAt` older than the newest `committedDate`, because a force-push re-pins `latestReviews.commit` to the new head — labels with GitHub colors, aggregate diffstat linking to the files tab — no per-file list by design).
...
- `mergePr` (`mergePullRequest`, method validated against repo-allowed set).
- `markPrReadyForReview` (own open drafts only, no confirmation modal — it is reversible).
- `updatePrBranch` (`updatePullRequestBranch` with `updateMethod` REBASE or MERGE — the preselected one comes from `githubControlCenter.updateBranch.defaultMethod`, REBASE out of the box, offered when the PR branch is behind base — detected via `baseRef.compare(headRef).behindBy`, NOT `mergeStateStatus`: that field only reports `BEHIND` under strict branch protection and shows `BLOCKED` otherwise).

### 7. Tree row context menu — `view/item/context`

- Copy PR URL; Copy Branch Name (`headRefName` is fetched in `PR_FIELDS`); Mute Repository / Mute Organization (append to `githubControlCenter.mutedRepos` at Global target; the config listener does the refresh).
...
### 11. AI brief — `brief/ai.ts` + `brief/codex.ts` CLI wrappers + `brief/briefPrompt.ts` pure prompt builder

- **What it is**: the details panel's "✨ Brief me" button generates a reviewer-oriented summary by spawning an AI CLI headless — zero runtime deps (`node:child_process`), no API key, the user's own subscription. Two backends, selected by `githubControlCenter.ai.backend`: **`claude-code`** (default, `brief/ai.ts`, `claude -p`) and **`codex`** (`brief/codex.ts`, `codex exec -`). Both share generic spawn plumbing (`useShell`/`quoteForShell`/`prepareSpawn`/`killChildTree`) extracted to `brief/spawn.ts`.
- **Backend DI is agnostic**: `DetailsSession` depends on `detectAi(): Promise<boolean>` and `runAiPrompt(systemPrompt, prompt): Promise<string>` — no command/model/backend in the signature. `extension.ts` is the only place that knows which backend is selected; its closures read `aiConfig().backend` on every call and dispatch to the Claude or Codex functions (imported under `detectClaudeCli`/`runClaudeCli` aliases to avoid a name clash with the agnostic DI keys). An unknown `backend` value resolves `detectAi` to `false` instead of falling through to either CLI.
- **Claude Code lockdown** (`brief/ai.ts`): `--tools ""` (PR content is untrusted — it must never execute anything); `--setting-sources ""` + `--system-prompt-file` + `cwd: os.tmpdir()` (verified empirically: without them the user's CLAUDE.md personas/hooks/memory leak into the output); prompt via stdin (never argv); `shell: true` on win32 only (npm `.cmd` shim; nothing untrusted enters the command line).
  - The system prompt travels as a temp file because win32's shell path cannot carry newlines in argv — written into a per-run `mkdtemp` directory (removed on settle): concurrent windows cannot swap each other's prompt and a predictable /tmp name cannot be planted by another local user.
  - The generation timeout kills the whole process tree on win32 (`taskkill /T`; `child.kill()` would only hit the cmd.exe wrapper); stdin carries a no-op error handler (a fast-exiting child EPIPEs the prompt write; the close handler owns the outcome).
- **Codex lockdown** (`brief/codex.ts`) — **best-effort, not a guarantee, unlike Claude's `--tools ""`**: Codex has no single flag that disables all tool execution; each built-in tool is disabled individually via `LOCKDOWN_ARGS` (`--sandbox read-only`, `--skip-git-repo-check`, `--ephemeral`, `--ignore-user-config`, `--ignore-rules`, `-c features.shell_tool=false`, `-c features.web_search=false`, `-c tools.view_image=false`, `-c project_doc_max_bytes=0`). **Accepted residual risk**: `--sandbox read-only` blocks writes, not reads, and Codex's file-read capability may be intrinsic to the agent — a maliciously-crafted PR could in principle make the model read a local file and echo it into the brief. This is documented (README, the `ai.codex.command` setting description) rather than solved; the Codex backend is opt-in (default stays `claude-code`) and the user accepts it as shared responsibility, the same way brief content already leaves the machine to a third-party model provider. `ponytail:` re-audit `LOCKDOWN_ARGS` whenever the Codex CLI is bumped — a new default-on tool widens the surface further.
  - No separate system-prompt flag: `runCodexPrompt` combines `` `${systemPrompt}\n\n${prompt}` `` into one message and sends it via the `codex exec -` stdin sentinel (never argv) — a fresh empty `mkdtemp` cwd per run avoids discovering a project `AGENTS.md`, but `--ignore-user-config` is confirmed to skip only `$CODEX_HOME/config.toml`, not project-doc discovery, hence the separate `project_doc_max_bytes=0` override.
- **Settings & trust**: every `githubControlCenter.ai.*` key is **machine-scoped** so an untrusted workspace cannot point the extension at an arbitrary binary, inject the response `language` into the trusted system prompt, or push an out-of-enum `model` onto the (win32) command line — `model` is additionally validated against its enum at read time (Claude only; Codex has no model setting yet).
  - `ai.backend` (enum: `claude-code`, `codex`) selects the backend and gates `refreshAiAvailability` — an unknown value disables AI instead of falling through to either CLI.
  - `ai.language` is backend-agnostic; `ai.claude.*` and `ai.codex.command` are backend-specific. VSCode has no conditional settings visibility, so both stay visible with a "\<backend\> only" note.
...
- **Load-bearing companions** (all live inside `DetailsSession`, not `extension.ts`):
  ...
  - `detectAi` runs **lazily on the first PR-panel open** (a window that never opens one spawns no `--version` probe of either CLI), with a timeout so a wedged CLI cannot leave availability undetermined; its results land behind a sequence counter (same pattern as `detailsRequestSequence`) so a slow stale probe never overwrites a newer config's availability.
  ...
  - An ai.*-only config change skips the GitHub poll; only an `ai.backend`/`ai.claude.command`/`ai.codex.command` change re-probes availability (language/model cannot affect it), via `session.refreshAiAvailabilityIfStarted()` — a no-op until the first panel open has started detection. AI UI hides entirely when `detectAi` fails.
...
- **Filters run before everything**: `applyFilters` (muted repos, hide drafts, hide reviewed) is applied to the snapshot before providers, badge, and trackers — lists, badge, and toasts must always agree. The badge counts the FILTERED lists.
